...
export function Mail() {
  ...
  if (accounts.length === 0) {
    return <p className="muted">Noch kein Mailkonto verbunden. Lege unter „Konten" eines an.</p>;
  }
  ...
  return (
    <div>
      <div className="row" style={{ marginBottom: "1rem" }}>
        ...
        <button className="primary" onClick={() => setDraft(emptyDraft())}>✎ Neue Mail</button>
        <span className="grow" />
        <button className="ghost" onClick={reload}>↻</button>
        <span className="label">Posteingang</span>
      </div>

      {err && <div className="err">{err}</div>}
      {loading && <p className="muted">Lade Nachrichten…</p>}

      {open ? (
        <div className="card" style={{ padding: "1.2rem" }}>
          <div className="row" style={{ marginBottom: "0.6rem" }}>
            <button className="ghost" onClick={() => setOpen(null)}>← Zurück</button>
            <span className="grow" />
            <button onClick={() => setDraft(replyDraft(open))}>↩ Antworten</button>
            <button onClick={() => setDraft(forwardDraft(open))}>↪ Weiterleiten</button>
          </div>
          <h2 style={{ marginBottom: "0.2rem" }}>{open.subject || "(kein Betreff)"}</h2>
          <div className="mail-from">{open.from} · {open.date}</div>
          <hr style={{ borderColor: "var(--self-line)", margin: "1rem 0" }} />
          <div style={{ whiteSpace: "pre-wrap", lineHeight: 1.55 }}>
            {open.text || open.html.replace(/<[^>]+>/g, "") || "(leer)"}
          </div>
        </div>
      ) : (
        <div className="mail-list">
          {messages.map((m) => (
            <div className={`mail-row ${m.seen ? "" : "unseen"}`} key={m.uid} onClick={() => openMsg(m.uid)}>
              <div>
                <div className="mail-subj">{m.subject || "(kein Betreff)"}</div>
                <div className="mail-from">{m.from}</div>
              </div>
              <div className="muted" style={{ fontSize: "0.78rem" }}>{m.date?.slice(0, 16)}</div>
            </div>
          ))}
          {!loading && messages.length === 0 && <p className="muted">Keine Nachrichten.</p>}
        </div>
      )}
